    ...
    def __init__(self, image_shape):
        ...
        if cfg.GAN.NETWORK_TYPE == "default":
            with tf.variable_scope("d_net"):
                ...
                self.d_fc_info_template = self.d_fc_info()
    ...
    def d_fc_info(self):
        ...
        template = \
            (pt.template("input").  # 128*9*4*4
             ...
             apply(leaky_rectify, leakiness=0.2).
             # custom_fully_connected(1))
             custom_conv2d(out_dim, k_h=self.s16, k_w=self.s16, d_h=self.s16, d_w=self.s16).
             flatten())

        return template

    def get_d_info(self, x_var):
        x_code = self.d_encode_img_template.construct(input=x_var)
        return self.d_fc_info_template.construct(input=x_code)
