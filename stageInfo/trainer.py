...
class CondGANTrainer(object):
    ...
    def sample_encoded_context(self, embeddings):
        ...
        return c, mean, cfg.TRAIN.COEFF.KL * kl_loss
    ...
    def init_opt(self):
        ...
        with pt.defaults_scope(phase=pt.Phase.train):
            with tf.variable_scope("g_net"):
                ...
                c, mean, kl_loss = self.sample_encoded_context(self.embeddings)
            ...
            if cfg.TRAIN.INFO:
                print('==> Use info loss')
                if 'embed' in cfg.CONFIG_NAME:
                    d_info_loss, g_info_loss = self.compute_info_loss(
                        self.images, fake_images, self.embeddings
                    )
                else:
                    d_info_loss, g_info_loss = self.compute_info_loss(
                        self.images, fake_images, mean
                    )

                discriminator_loss += d_info_loss
                generator_loss += g_info_loss

                self.log_vars.append(("d_info_loss", d_info_loss))
                self.log_vars.append(("g_info_loss", g_info_loss))
    ...
    def sampler(self):
        c, _, _ = self.sample_encoded_context(self.embeddings)
        if cfg.TRAIN.FLAG:
    ...
    ## info loss
    def compute_info_loss(self, images, fake_images, target_info):
        d_info_loss = 0.0
        if 'dfake' in cfg.CONFIG_NAME:
            d_info_loss += tf.reduce_mean(tf.losses.mean_squared_error(
                labels=target_info, predictions=self.model.get_d_info(fake_images)
            ))
        if 'dreal' in cfg.CONFIG_NAME:
            d_info_loss += tf.reduce_mean(tf.losses.mean_squared_error(
                labels=target_info, predictions=self.model.get_d_info(images)
            ))
            
        g_info_loss = tf.reduce_mean(tf.losses.mean_squared_error(
            labels=target_info, predictions=self.model.get_d_info(fake_images)
        ))
        return d_info_loss, g_info_loss
    ...
    def compute_losses(self, images, wrong_images, fake_images, embeddings):
        ...
        wrong_d_loss = tf.nn.sigmoid_cross_entropy_with_logits(
            logits=wrong_logit, labels=tf.zeros_like(wrong_logit)
        )
        ...
        fake_d_loss = tf.nn.sigmoid_cross_entropy_with_logits(
